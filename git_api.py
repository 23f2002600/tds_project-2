...
def GA2_3(question):
    ...
    github_replace_text(
        token=token,
        repo="sanjitaprabhu/website",
        file_path="index.html",
        pattern=pattern,
        replacement=email
    )
    print("Email updated in index.html")
    trigger_github_workflow(token=token, repo="sanjitaprabhu/website",
                            workflow_file="daily_commit.yml")  # Trigger the workflow after
    time.sleep(15)
    return "https://sanjitaprabhu.github.io/website/?v=2"
...
async def GA2_6_file(file: UploadFile = File(...)):
    ...
    response = github_write_file(
        token, "sanjitaprabhu/api", file_path_on_github, file_content)
...
async def GA2_9_file(file: UploadFile = File(...)):
    ...
    # Upload the file to GitHub
    response = github_write_file(
        token, "sanjitaprabhu/tds_ga2_9", file_path_on_github, file_content)
...
def GA2_7(question):
    ...
    trigger_github_workflow(
        token=token, repo="sanjitaprabhu/Test", workflow_file="Daily_Commit.yml")
    time.sleep(15)
    return "https://github.com/sanjitaprabhu/Test"

def GA4_8(question):
    return GA2_7(question)

# GA1_13("""pre-commit: Git hooks
# Let's make sure you know how to use GitHub. Create a GitHub account if you don't have one. Create a new public repository. Commit a single JSON file called email.json with the value {"email": "23f2002600@ds.study.iitm.ac.in"} and push it.
...
# GA2_3("""Publish a page using GitHub Pages that showcases your work. Ensure that your email address 23f2002600@ds.study.iitm.ac.in is in the page's HTML.

# GitHub pages are served via CloudFlare which obfuscates emails. So, wrap your email address inside a:

# <!--email_off-->23f2002600@ds.study.iitm.ac.in<!--/email_off-->
# What is the GitHub Pages URL? It might look like: https://[USER].github.io/[REPO]/""")

# GA2_7("""Create a GitHub action on one of your GitHub repositories. Make sure one of the steps in the action has a name that contains your email address 23f2002600@ds.study.iitm.ac.in. For example:

# jobs:
#   test:
#     steps:
#       - name: 23f2002600@ds.study.iitm.ac.in
...
# Include a step with your email 23f2002600@ds.study.iitm.ac.in in its name
